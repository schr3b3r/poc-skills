...
1. **Scaffold:** The script initializes a new SvelteKit project (using the standard Svelte template) and installs necessary dependencies (e.g., Chart.js for visualizations).
2. **Data Ingestion:** Once set up, instruct the user (or automate it yourself) to copy the generated `combined_data.json` or other JSON outputs from the `*-vitals` skills into the new dashboard's `src/lib/data` or `static/data` folder.
3. **Run:** Start the dev server.
   ```bash
   cd <target-directory>
   npm run dev
   ```
